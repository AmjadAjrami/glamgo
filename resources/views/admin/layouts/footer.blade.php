...
<script>
    ...
    function displayBrowseWindow() {
        document.getElementById('files_input').click();
    }

    function displayNewImage(event) {
        let input = event.target;
        let output = document.getElementById('image_display_tag');
        if (input.files.length) {
            ...
            output.src = '{{url('/')}}' + '/storage/Logo.jpeg';
        }
    }
    ...
    function get_notifications(){
        ...
        $.ajax({
            url: '{{ url(app()->getLocale() . '/tmg/get_notifications/') }}/',
            method: 'GET',
            success: function (data) {
                ...
                $.each(data.notifications, function (index, value){
                    var href = '';
                    var user_name = '';
                    if(value.type == 'chat_support_notification'){
                        user_name = value.user_name;
                        href = '{{ url(app()->getLocale() . '/tmg/support') }}';
                    }
                    if(value.type == 'order_notification'){
                        user_name = value.user_name;
                        href = '{{ url(app()->getLocale() . '/tmg/orders') }}';
                    }
                    if(value.type == 'contact_us_notification'){
                        user_name = '{{ app()->getLocale() == 'ar' ? 'طلب مساعدة' : 'Help Request' }}';
                        href = '{{ url(app()->getLocale() . '/tmg/contact_us') }}';
                    }
                    html += '<a class="d-flex" href="'+ href +'">' +
                        ...
                        '<p class="media-heading"><span class="fw-bolder">'+ user_name +'</span>' +
    ...
    $('#notification_btn').on('click', function (){
        ...
        $.ajax({
            url: '{{ url(app()->getLocale() . '/tmg/read_notifications/') }}/',
            method: 'GET',
            success: function (data) {

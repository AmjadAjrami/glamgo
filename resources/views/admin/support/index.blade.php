                            ...
                            <div class="d-flex align-items-center w-100">
                                <div class="sidebar-profile-toggle">
                                    <div class="avatar avatar-border">
                                        <img src="{{ auth('admin')->user()->image }}" alt="user_avatar" height="42"
                                             width="42"/>
                                    </div>
                                    @lang('common.admin') : {{ auth('admin')->user()->name }}
                                </div>
                            </div>
                        </div>
                        <!-- Sidebar header end -->
                        ...
                        <div id="users-list" class="chat-user-list-wrapper list-group">
                            ...
                            <ul class="chat-users-list chat-list media-list">
                                @foreach($supports as $support)
                                    <li data-key="{{ $support->firebase_id }}"
                                        class="chat_item {{ $support->user_id == request('user_id') ? 'active' : '' }}"
                                        data-user_id="{{ $support->user_id }}">
                                        <span class="avatar"><img src="{{ @$support->user->image }}"
                                                                  height="42" width="42"/>
                                        </span>
                                        <div class="chat-info flex-grow-1">
                                            <h5 class="mb-0">{{ @$support->user->name }}</h5>
                                            <p class="card-text text-truncate">
                                                {{ $support->last_message }}
                                            </p>
                                        </div>
                                        <div class="chat-meta text-nowrap">
                                            <small
                                                class="float-end mb-25 chat-time">{{ $support->last_message_date }}</small>
                                            @if($support->admin_unread_messages > 0)
                                                <span
                                                    class="badge bg-danger rounded-pill float-end">{{ $support->admin_unread_messages }}</span>
                                            @endif
                                        </div>
                                    </li>
                                @endforeach
...
@section('scripts')
    ...
    <script>
        ...
        $('.chat_item').on('click', function () {
            ...
            var user_image = '';
            $.ajax({
                url: "{{ url(app()->getLocale() . '/tmg/support/user_details/') }}/" + user_id,
                type: "GET",
                cache: false,
            ...
            database.ref('support').child(key).on('value', function (querySnapshot) {
            ...
            database.ref('support').child(key).on('value', function (querySnapshot) {
                ...
                $.ajax({
                    url: "{{ url(app()->getLocale() . '/tmg/support/update_message_read/') }}/" + key,
                    type: "PUT",
                    data: {
                        _token: '{{ csrf_token() }}',
                    },
                ...
                if (querySnapshot.exists() == true) {
                    querySnapshot.forEach(function (child) {
                        ...
                        if (messages.idSender == '{{ auth('admin')->id() }}' && messages.userType == 'admin') {
                            if (messages.type == 'text') {
                                html += '<div class="chat chat-left">' +
                                    '<div class="chat-avatar">' +
                                    '<span class="avatar box-shadow-1 cursor-pointer">' +
                                    '<img src="{{ auth('admin')->user()->image }}" alt="avatar" height="36" width="36"/>' +
                                    '</span>' +
                                    '</div>' +
                                    '<div class="chat-body">' +
        ...
        $(document).on('change', '#change_language', function () {
            ...
            {{--$.ajax({--}}
            {{--    url: "{{ url(app()->getLocale() . '/salon/chats/translation_messages') }}",--}}
            {{--    type: "GET",--}}
            {{--    data: {--}}
        ...
        function send_message() {
            ...
            if (/\S/.test(message)) {
                ...
                var firebase_id = 'Support-UserID-' + user_id;

                $.ajax({
                    url: "{{ url(app()->getLocale() . '/tmg/support/translation_message') }}",
                    type: "GET",
                    data: {
                        message: message,
                        _token: '{{ csrf_token() }}',
                    ...
                    cache: false,
                    success: function (dataResult) {
                        database.ref('support').child(firebase_id).push().set({
                            idSender: {{ auth('admin')->id() }},
                        ...
                        $.ajax({
                            url: "{{ url(app()->getLocale() . '/tmg/support/send_message_notification') }}",
                            type: "GET",
                            data: {
                                user_id: user_id,
                                message: message,

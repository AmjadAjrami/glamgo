                            ...
                            <div class="d-flex align-items-center w-100">
                                <div class="sidebar-profile-toggle">
                                    <div class="avatar avatar-border">
                                        <img src="{{ auth('salon')->user()->image }}" alt="user_avatar" height="42"
                                             width="42"/>
                                    </div>
                                    @lang('common.salon') : {{ auth('salon')->user()->name }}
                                </div>
                            </div>
                        </div>
                        <!-- Sidebar header end -->
...
@section('scripts')
    ...
    <script>
        ...
        var key = '{{auth('salon')->id()}}-SalonID-' + user_id + '-UserID';
        ...
        // $('.chat_item').on('click', function () {
            ...
            database.ref('chat').child(key).on('value', function (querySnapshot) {
                ...
                $.ajax({
                    url: "{{ url(app()->getLocale() . '/salon/chats/update_message_read/') }}/" + key,
                    type: "PUT",
                    data: {
                        _token: '{{ csrf_token() }}',
                    },
                ...
                if (querySnapshot.exists() == true) {
                    querySnapshot.forEach(function (child) {
                        ...
                        if (messages.idSender == '{{ auth('salon')->id() }}' && messages.userType == 'salon') {
                            if (messages.type == 'text') {
                                html += '<div class="chat chat-left">' +
                                    '<div class="chat-avatar">' +
                                    '<span class="avatar box-shadow-1 cursor-pointer">' +
                                    '<img src="{{ auth('salon')->user()->image }}" alt="avatar" height="36" width="36"/>' +
                                    '</span>' +
                                    '</div>' +
                                    '<div class="chat-body">' +
        ...
        function send_message() {
            ...
            if (/\S/.test(message)) {
                ...
                var firebase_id = '{{auth('salon')->id()}}-SalonID-' + user_id + '-UserID';

                $.ajax({
                    url: "{{ url(app()->getLocale() . '/salon/chats/translation_message') }}",
                    type: "GET",
                    data: {
                        message: message,
                        _token: '{{ csrf_token() }}',
                    ...
                    success: function (dataResult) {
                        ...
                        $.ajax({
                            url: "{{ url(app()->getLocale() . '/salon/chats/send_message_notification') }}",
                            type: "GET",
                            data: {
                                user_id: user_id,
                                message: message,

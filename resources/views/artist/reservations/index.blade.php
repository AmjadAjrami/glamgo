...
@section('scripts')
    <script>
        var url = '{{ url(app()->getLocale() . "/artist/reservations") }}/';
        ...
        var dt_adv_filter = $('#datatable').DataTable({
            ...
            columns: [
                ...
                {
                    "render": function (data, type, full, meta) {
                        return '<a class="btn btn-info" href="{{ url(app()->getLocale() . '/artist/reservations/user_chat/') }}/'+ full.user_id +'">@lang('common.chat')</a>';
                    }, orderable: false, searchable: false
                },
                {
                    "render": function (data, type, full, meta) {
        ...
        $(document).on('change', '.reservation_status', function () {
            ...
            if (status == 5) {
                ...
                $('#editUserForm').attr('action', '{{ url(app()->getLocale() . '/artist/reservations/update_status') }}');
                ...
                $.ajax({
                    url: '{{ url(app()->getLocale() . '/artist/reservations/update_status') }}',
                    method: 'PUT',
                    data: {
                        reservation_id: id,
                        status: status,
        ...
        $(document).on('click', '.reservation_details', function () {
            ...
            $.ajax({
                url: '{{ url(app()->getLocale() . '/artist/reservations/reservation_details') }}',
                method: 'GET',
                data: {
                    reservation_id: id,
                },
                success: function (data) {
                    ...
                    $('#print').attr('href', '{{ url('/artist/reservations/ticket/') }}/' + data.reservation.id);
                }
            });
        });

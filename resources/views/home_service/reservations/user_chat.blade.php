                    ...
                    <div class="sidebar-content">
                            ...
                            <div class="d-flex align-items-center w-100">
                                <div class="sidebar-profile-toggle">
                                    <div class="avatar avatar-border">
                                        <img src="{{ auth('home_service')->user()->image }}" alt="user_avatar" height="42"
                                             width="42"/>
                                    </div>
                                    @lang('common.home_service') : {{ auth('home_service')->user()->name }}
                                </div>
                            </div>
                        </div>
                        <!-- Sidebar header end -->
                        ...
                        <div id="users-list" class="chat-user-list-wrapper list-group">
                            ...
                            <ul class="chat-users-list chat-list media-list">
                                <li class="chat_item active">
                                        <span class="avatar"><img src="{{ @$user->image }}"
                                                                  height="42" width="42"/>
                                        </span>
                                    ...
                                    <div class="chat-meta text-nowrap">
                                        <small
                                            class="float-end mb-25 chat-time">{{ @$chat->last_message_date == null ? '' : \Carbon\Carbon::parse(@$chat->last_message_date)->format('Y-m-d A g:i') }}</small>
                                    ...
                                    <header class="chat-header">
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div class="avatar avatar-border m-0 me-1">
                                                <img id="user_image" src="{{ $user->image }}"
                                                     alt="avatar" height="36" width="36"/>
                                            </div>
                                            @lang('common.user') : <h6 class="mb-0"
                                                                       id="user_name">{{ $user->name }}</h6>
                                        </div>
...
@section('scripts')
    ...
    <script>
        ...
        var key = '{{auth('artist')->id()}}-ArtistID-' + user_id + '-UserID';
        ...
        // $('.chat_item').on('click', function () {
            ...
            database.ref('chat').child(key).on('value', function (querySnapshot) {
                if (querySnapshot.exists() == true) {
                    querySnapshot.forEach(function (child) {
                        ...
                        if (idSender != '{{ auth('home_service')->id() }}') {
            ...
            database.ref('chat').child(key).on('value', function (querySnapshot) {
                ...
                $.ajax({
                    url: "{{ url(app()->getLocale() . '/h-service/chats/update_message_read/') }}/" + key,
                    type: "PUT",
                    data: {
                        _token: '{{ csrf_token() }}',
                    },
                ...
                if (querySnapshot.exists() == true) {
                    querySnapshot.forEach(function (child) {
                        ...
                        if (messages.idSender == '{{ auth('home_service')->id() }}' && messages.userType == 'home_service') {
                            if (messages.type == 'text') {
                                html += '<div class="chat chat-left">' +
                                    '<div class="chat-avatar">' +
                                    '<span class="avatar box-shadow-1 cursor-pointer">' +
                                    '<img src="{{ auth('home_service')->user()->image }}" alt="avatar" height="36" width="36"/>' +
                                    '</span>' +
                                    '</div>' +
                                    '<div class="chat-body">' +
                            ...
                            if (messages.type == 'text') {
                                html += '<div class="chat">' +
                                    '<div class="chat-avatar">' +
                                    '<span class="avatar box-shadow-1 cursor-pointer">' +
                                    '<img class="user_profile_image" src="{{ $user->image }}" alt="avatar" height="36" width="36"/>' +
                                    '</span>' +
                                    '</div>' +
                                    '<div class="chat-body">' +
                                    '<div class="chat-content">' +
        ...
        function send_message() {
            ...
            if (/\S/.test(message)) {
                ...
                var firebase_id = '{{auth('home_service')->id()}}-HomeServiceID-' + user_id + '-UserID';

                $.ajax({
                    url: "{{ url(app()->getLocale() . '/h-service/chats/translation_message') }}",
                    type: "GET",
                    data: {
                        message: message,
                        _token: '{{ csrf_token() }}',
                    ...
                    success: function (dataResult) {
                        ...
                        $.ajax({
                            url: "{{ url(app()->getLocale() . '/h-service/chats/send_message_notification') }}",
                            type: "GET",
                            data: {
                                user_id: user_id,
                                message: message,

...
class FinancialAccountsController extends Controller
{
    ...
    public function index()
    {
        ...
        $reservations_price = Reservation::query()->where('home_service_id', $home_service->id)->where('payment_method_id', '!=', 1)->where('status', 3)->whereNull('total_price_after_code')->sum('total_price');
        $reservations_price_with_discount = Reservation::query()->where('home_service_id', $home_service->id)->where('payment_method_id', '!=', 1)->where('status', 3)->whereNotNull('total_price_after_code')->sum('total_price_after_code');

        $total_amount = $reservations_price + $reservations_price_with_discount;

        $withdraws = Withdraw::query()->where('home_service_id', $home_service->id)->where('type', 1)->where('status', 1)->sum('amount');
        $cash_withdraws = Withdraw::query()->where('home_service_id', $home_service->id)->where('type', 2)->where('status', 1)->sum('amount');
        ...
        $reservations_cash = Reservation::query()->where('home_service_id', auth('home_service')->id())
        ...
        $home_service_withdraws = Withdraw::query()->where('home_service_id', $home_service->id)->where('type', 1)->get();
        $home_service_cash_transfers = Withdraw::query()->where('home_service_id', $home_service->id)->where('type', 2)->get();

        return view('home_service.financial_accounts.index', compact('details', 'home_service_withdraws', 'home_service_cash_transfers'));
    }
